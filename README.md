...
`ls cd pwd cat echo touch mkdir rm mv cp chmod chown grep find du ps kill sudo
whoami help`, plus pipes (`|`), redirects (`>`, `>>`), quoting, `~` and `..`,
and command history on ↑/↓.
...
```
engine/   isomorphic game core - vfs, shell, missions (runs in browser AND node)
server/   http + websockets, auth, elo, race matchmaking, chaos rooms
client/   the terminal UI
test/     engine + full websocket integration tests
```
